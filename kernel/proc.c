...
found:
  ...
  // An initialized kernel page table.
  p->kpagetable = kvminitnew();
  if (p->kpagetable == 0) {
  ...
  // Set up the mapping for the processes' kernel stack.
...
// Free a process's kernel page table without freeing
// the physical memory it refers to.
void
proc_freekpagetable(pagetable_t kpagetable)
{
  freetable(kpagetable);
}
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        c->proc = p;
        kvmswtchpgtbl(p->kpagetable);
        swtch(&c->context, &p->context);

        // Process is done running for now.
        // It should have changed its p->state before coming back.
        c->proc = 0;
        kvminithart(); // not kvminit()
